...
@section('js_scripts')
  <script>
        ...
        $("#submit_tambah").submit(function(e) {
            ...
            } else if($("#sesi_kunjungan").val() == '') {
                iziToast.warning({
                    title: 'WARNING !',
                    message: 'Sesi Kunjungan Tidak Boleh Kosong',
                    position: 'topRight'
                });
            }else if($("#audio").val() == '') {
                iziToast.warning({
    ...
    function edited(id){
        $.ajax({
            ...
            success: function(result)
            {
                ...
                $('[name="nomor_urut"]').val(result.data.nomor_urut);
                $('[name="audio"]').val("");
        ...
        let id_prov = $('select[name="sesi_kunjungan"]').val();


        $.ajax({
            url: '/dashboard/kelola_antrian_layanan_kunjungan/'+id, //akses url 
            tye: "get", //tipe method
            ...
            success: function(response) {
                ...
                for (i = 0; i < response.length; i++) {
                    ...
                    html = html + 
                    `<option value="pertama" ${response[i].sesi_kunjungan == 'pertama' ? 'selected' : ''}>Pertama</option>
                    <option value="kedua" ${response[i].sesi_kunjungan == 'kedua' ? 'selected' : ''}>Kedua</option>
                    <option value="ketiga" ${response[i].sesi_kunjungan == 'ketiga' ? 'selected' : ''}>Ketiga</option>`;
                }

                $('select[name="sesi_kunjungan"]').html(
                    '<option>Pilih Sesi Kunjungan</option>'+
                    html
                );
            }
        })
                      ...
                      <div class="modal-body">
                              ...
                              <div class="form-group">
                                <label for="sesi_kunjungan" 
                                    class="control-label">Sesi Kunjungan:</label>
