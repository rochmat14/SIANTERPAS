...
function edited(id){
    ...
    $.ajax({
        ...
        success: function(result)
        {
            ...
            $('[name="subtitle"]').val(result.data.subtitle);
            $('[name="button_text"]').val(result.data.button_text);
            $('[name="url"]').val(result.data.url);
            $('[name="target"]').val(result.data.target);
                    ...
                    <div class="modal-body">
                        <form id="form" method="post" class="form-horizontal">
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="subtitle" placeholder="Sub Title"></textarea>
                            </div>
                            <div class="form-group">
                                <label for="button_text" class="control-label">Text Button :</label>
                                <input type="text" class="form-control" id="button_text" name="button_text" placeholder="text button">
                            </div>
                            <div class="form-group">
                                <label for="url" class="control-label">URL Slider :</label>
                                <input type="text" class="form-control" id="url" name="url" placeholder="URL Slider">
                            </div>

                            <div class="form-group">
                                <label for="target" class="control-label">URL Target :</label>
                                <select name="target" class="form-control">
                                    <option value="1">URL Target Self - Tetap Pada Halaman Yang Sama</option>
                                    <option value="2">URL Target Blank - Pindah pada tab baru</option>
                                </select>
                            </div>

                            <div class="form-group">
                                <label for="image" class="control-label">Sliders Images:</label>
                                <input type="file" class="form-control" id="image" name="image" placeholder="Sliders Images">
                                <span><i>Upload Image Rekomendasi Ukuran 1920 x 645 px</i></span>
                            </div>

                            <div class="form-group">
                                <label for="image_mobile" class="control-label">Sliders Images Mobile:</label>
                                <input type="file" class="form-control" id="image_mobile" name="image_mobile" placeholder="Sliders Images Mobile">
                                <span><i>Upload Image Rekomendasi Ukuran 184 x 80 px</i></span>
                            </div>
                        </form>
                    </div>
