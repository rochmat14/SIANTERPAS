...
@section('content')
        ...
        <div class="container text-center">
            <div class="row">
                ...
                <div class="col-xl-8 col-md-8 col-lg-8">
                    ...
                    <form action="{{ route('sesiKetiga.store') }}" method="POST">
                        @csrf
                        <?php 
                        $count=0;
                        ;?>
                        @foreach($nomorAntrianSesiKetiga as $data)
                        <div class="card">
                            <div class="card-body">
                                <h1>Nomor Urut Sesi Ketiga</h1>
                                <h1>
                                {{$data->nomor_urut}}
                                </h1>
                        ...
                        <a onclick="lKunjungan3()" class="btn btn-primary btn-lg col-xl-12 col-md-12 col-lg-12 mb-4 fw-bold">PANGGIL</a>
                    </form>
                </div>
                <div class="col-xl-2 col-md-2 col-lg-2"></div>
            </div>
    ...
    <script>
        function lKunjungan3(){
            var mp3 = <?php echo json_encode($data->audio); ?>;
            var audio = mp3.slice(0,3);
            var nomor_urut = <?php echo json_encode($data->nomor_urut); ?>;

            if(audio == nomor_urut) {
                var audio = new Audio("/audio/layanan_kunjungan/sesi_ketiga/"+<?php echo json_encode($data->nomor_urut); ?>+".mp3");
                audio.play();
            }else{
